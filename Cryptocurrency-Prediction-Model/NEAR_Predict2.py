...
def get_binance_data(symbol='BTC/USDT', since='2014-10-12', limit=1000):
    exchange = ccxt.binance()  # 또는 ccxt.bithumb()
    ...
    while True:
        ...
        if not ohlcv:
            break
        all_ohlcv += ohlcv
        since_ms = ohlcv[-1][0] + 24 * 60 * 60 * 1000  # move to next day
        if len(ohlcv) < limit:
            break
...
btc_data = get_binance_data("BTC/USDT" , since='2022-10-12')
...
# 비트코인 데이터 다운로드
# end_date = datetime.today().strftime('%Y-%m-%d')
# btc_data = yf.download('BTC-USD', start='2018-01-01', end="2019-5-10",interval='1d')
